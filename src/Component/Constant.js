...
export const ExpandInfo = ({record, setExpended, expanded}) => {
    console.log(record?.expanded);
    return (
      <Fragment>
          <div onClick={() => setExpended(record?.expanded ? '': record?.record?.Country )} className='expand-column'>
            {record?.expanded ? <MinusCircleOutlined /> :  <PlusCircleOutlined />} &nbsp; &nbsp;{record?.record?.Country}
          </div>
      </Fragment>
    )
  }
