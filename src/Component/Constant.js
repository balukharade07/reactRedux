...
export const ExpandInfo = ({ record, setExpended, expanded }) => {
  ...
  return (
    <Fragment>
      <div
        onClick={() =>
          setExpended(record?.expanded ? "" : record?.record?.Country)
        }
        className="expand-column"
      >
        {record?.expanded ? <MinusCircleOutlined /> : <PlusCircleOutlined />}{" "}
...
export function getRootURL(subUrl){
    return `http://localhost:5000/${subUrl}`
}
