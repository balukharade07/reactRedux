...
import { Col, Row, Form, Collapse, Input, Button, Modal, Space, message, Popconfirm } from "antd";
...
const Quote = () => {
    ...
    const getQuote = () => {
        axios.get(`http://localhost:5000/getQuote/${_id}`)
            .then((response) => {
    ...
    const handleOk = async (item) => {
        if (item) {
            return await form.current.validateFields().then(values => {
                axios.put(`http://localhost:5000/editQuote/${item._id}`, { quote: values.quote })
                    .then((response) => {
        ...
        return await form.current.validateFields().then(values => {
            console.log(values);
            axios.post('http://localhost:5000/addQuote', values)
                .then((response) => {
    ...
    const genExtra = (item) => (
        <>
            <EditTwoTone title="Edit" onClick={(event) => {
                event.stopPropagation();
                modal.confirm(config(item));
            }} />
            <Popconfirm title="Sure to Delete?" onCancel={(event) => {
                event.stopPropagation();
            }} onConfirm={(event) => {
                event.stopPropagation();
                handleDelete(item._id)
            }}>
                <DeleteTwoTone title="Delete" onClick={(event) => {
                    event.stopPropagation();
                }} />
            </Popconfirm>
    ...
    return (
        <>
            <div>
                <Row>
                    <Col span={20}><h2>Your Quote</h2></Col>
                    <Col span={4} style={{ textAlign: 'right' }}>
                        <ReachableContext.Provider value="Light">
                            <Space>
                                <Button onClick={() => {
                                    modal.confirm(config());
                                }} type="primary">Create Quote </Button>
                            </Space>
                            {contextHolder}
                ...
                <div style={{ marginTop: '10px' }}>
                    <Collapse expandIcon={false} accordion={false} collapsible='header'>
                        {quoteData?.map(item => {
                            return <Collapse.Panel key={item._id} showArrow={false} header={item.quote} extra={genExtra(item)} />
                        })}
                    </Collapse>
                </div>
            </div>
